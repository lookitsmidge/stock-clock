...
public class encryptionTest {
	...
	//split the message
		...
		for(int i=0; i<sentO.length();i++) {//this loops through the message and encrypts it
			Lock_message = Lock_message + (""+  (char)(   (int)sentO.charAt(i)+i+1)  );// this converts the charachter currently selected to be a integer and then adds 3 to it and then converts it back to a char then to a string and then adds it to the message
		}
		sentO = Lock_message;
	...
	public String Decrypt(String parameter) {
		...
		for(int i=0; i<parameter.length();i++) {//this loops through the message and encrypts it
			...
			Lock_message = Lock_message + (""+(char)( (int)(parameter.charAt(i)-i-1)));// this converts the charachter currently selected to be a integer and then adds 3 to it and then converts it back to a char then to a string and then adds it to the message
